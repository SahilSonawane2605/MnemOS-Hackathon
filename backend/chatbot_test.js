...
const { getKey, rotateKey, totalKeys } = require("./keyManager");
...
const MOCK_MEMORIES = [
  {
    core_interest: "Docker Containerization",
    user_goal: "Learning Docker troubleshooting",
    ai_summary: "The user explored Docker basics and solved exit code 137 memory issues.",
    timestamp: "2026-06-09T10:00:00Z",
  },
  {
    core_interest: "Web Development",
    user_goal: "Building responsive layouts using CSS Grid",
    ai_summary: "The user fixed alignment bugs and studied responsive breakpoints.",
    timestamp: "2026-06-09T14:30:00Z",
  },
  {
    core_interest: "Python Programming",
    user_goal: "Working on computer vision and hand gesture detection",
    ai_summary: "The user experimented with OpenCV and MediaPipe.",
...
let ai = new GoogleGenAI({ apiKey: getKey() });
let chatHistory = [];
...
function promptUser() {
  rl.question("\n💬 You: ", async (input) => {
    ...
    const memories = getRelevantMemories(userInput);
    ...
    if (!responseText) {
      console.log("\n🤖 MnemOS: I'm unable to respond right now. Please try again.");
      promptUser();
      return;
    }

    console.log(`\n🤖 MnemOS: ${responseText}`);
    appendToHistory("MnemOS", responseText);
...
console.log("\n╔══════════════════════════════════════╗");
console.log("║     🧠  MnemOS Memory Core Online     ║");
console.log("╚══════════════════════════════════════╝");
